...
figS1 = lapply(weeks, countyMapPlot, county_rt = county_rt, supp = TRUE)
...
jpeg("output/plot/fig_spatial.jpeg",
     width = 10*600, height = 7.5*600, res = 600)
pdf("output/plot/fig_spatial.pdf",
     width = 10, height = 7.5)
pSPCorr
dev.off()
...
# Figure S21 - Bootstrap counties

```{r fig s21, include=FALSE,  warning=FALSE, message=FALSE}

getEst <-function(model.used) {# 1 = duration; 2 = categorical
  ...
  esti = summary(model.used)$coefficients
  ci = cbind(esti[,1] - 1.96*esti[ ,2],
             esti[,1] + 1.96*esti[ ,2])
  rownames(ci) = rownames(esti)
...
getBootstrapData = function(data, FIPS, nCounty){
  
  fips_new = sample(FIPS,
                    size = nCounty,
                    replace = TRUE)
  
  data.new = lapply(fips_new, function(i){
    
    data[data$FIPS == i, ]
    
  })
  data.new = do.call('rbind', data.new)
  
  data.new = data.new[ ,!(colnames(data.new) %in% c("date_all_school_closed",
                                                    "date_partial_school_closed"))]
  data.new = data.new[rowSums(is.na(data.new)) == 0, ] 
  
  model.new = modelFit(model = 'main',
                       dat = data.new,
                       stateLevel = FALSE)
  
  est.new = getEst(model.new)
  
  est.new
  
}

est.bootstrap = lapply(1:100, function(x) getBootstrapData(data = data.google,
                                                           FIPS = FIPS,
                                                           nCounty = length(FIPS)))

est.bootstrap = do.call('rbind', est.bootstrap)

smr.est.bootstrap = est.bootstrap %>%
  group_by(intervention) %>%
  summarise(Q50 = quantile(coefficients, 0.5),
            Q025 = quantile(coefficients, 0.025),
            Q975 = quantile(coefficients, 0.975)) %>%
  mutate(Model = 'Bootstrap') %>%
  rename(coefficients = Q50,
         ci.lower = Q025,
         ci.upper = Q975)

smr.est.bootstrap = rbind(smr.est.bootstrap,
                          est[est$Model == 'GEE',
                              -ncol(est)])

smr.est.bootstrap$Model[smr.est.bootstrap$Model == 'GEE'] = 'Main'

pBootstrap <- ggplot(data = smr.est.bootstrap,
                aes(y = factor(intervention,
                               levels = intv.order),
                    x = coefficients,
                    color = Model,
  ...
  theme(axis.text.x = element_text(angle = 0, 
        ...
        legend.title = element_text(size = 14)) +
  scale_color_manual(name = "Model",
                     values = c( "#9b9fa2", "#d9534f"),
                     labels = c('Bootstrap', 'Main')) +
  scale_y_discrete(labels = labels.order) +
    scale_x_continuous(breaks = seq(0.4, 1.4, 0.2),
                       labels = c('-60', '-40', '-20', '0', '20', '40'),
                       limits = c(0.4, 1.4), 
...
# Figure 25 - Fixed effect of county
```{r}
...
jpeg("output/plot/fig_s25.jpeg",
     width = 10*600, height = 7.5*600, res = 600)
pdf("output/plot/fig_s25.pdf",
     width = 10, height = 7.5)
figFixed
dev.off()
...
tabForRsq = function(models){
  ...
  tab[3 , 2:5] = c('-',
                   '-',
                   "Weeks since county’s first case",
                   "Weeks since county’s first case")
...
summary_res = read.csv('/Volumes/GoogleDrive/My Drive/R projects/Coronavirus/covidIntervention/public/data/summary_estimates_rt_each_run.csv',
                       header = TRUE,
                       stringsAsFactors = FALSE)

figIndiRtEst = ggplot(data = summary_res,
       aes(y = factor(intervention,
                      levels = intv.order),
           x = Q50,
           color = Model,
...
jpeg('output/plot/individual_rt_est.jpeg', width = 10*600, height = 8.3*600, res = 600)
# pdf('output/plot/individual_rt_est.pdf', width = 10, height = 8.3)
figIndiRtEst
...
names(labels.order)[14] = 'earliest_date_school_closed'
intv.order[14] = 'earliest_date_school_closed'

modelFitSpatial = function(model, dat){
...
modelsSpatial = lapply(c('main', 'spatial'), modelFitSpatial, dat = dat)

estSpatial <- lapply(1:2,
                     function(model){# 1 = gee; 2 = log-lm
 ...
 nm = c("log10_popsize",
   ...
   type = c('main', 'spatial')[model]

  esti = summary(modelsSpatial[[model]])$coefficients
  ci = cbind(esti[,1] - 1.96*esti[ ,2],
             esti[,1] + 1.96*esti[ ,2])
  rownames(ci) = rownames(esti)
...
estSpatial = do.call('rbind', estSpatial)


pCompSpatialCorr <- ggplot(data = estSpatial,
                aes(y = factor(intervention,
                               levels = intv.order),
                    x = coefficients,
                    color = Model,
...
jpeg('output/plot/spatial_corr_gee.jpeg', width = 10*600, height = 8.3*600, res = 600)
# pdf('output/plot/spatial_corr_gee.pdf', width = 10, height = 8.3)
pCompSpatialCorr
